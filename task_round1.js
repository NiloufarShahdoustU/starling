...
export async function runTaskRound1(jsPsych, trialNumberIterate_input) {
  // const jsPsych = initJsPsych();  // Initialize jsPsych here if it's not initialized elsewhere
  const timeline = [];
  ...
  await new Promise((resolve) => {
    jsPsych.on_finish = function () {
      resolve();  // Resolve the promise when the experiment finishes
    };
  });
